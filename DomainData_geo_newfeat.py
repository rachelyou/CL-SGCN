...
class DomainData(InMemoryDataset):
    ...
    def raw_file_names(self):
        return ["feat.txt", "3cor.txt", "edgelist.txt", "labels.txt"]
    ...
    def process(self):
        ...
        docs_path = osp.join(self.raw_dir, '{}_feat.txt'.format(self.name))
        f = open(docs_path, 'rb')
        content_list = []
        ...
        cor_path = osp.join(self.raw_dir, '{}_3cor.txt'.format(self.name))
        f = open(cor_path, 'rb')
        content_list = []
        for line in f.readlines():
            line = str(line, encoding="utf-8")
            content_list.append(line.split(","))
        pos = np.array(content_list, dtype=float) ##
        ...
        data_list = []
        data = Data(edge_index=edge_index, x=x, pos=pos, y=y)
